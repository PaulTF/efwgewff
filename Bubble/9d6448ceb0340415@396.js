  ...
  main.variable(observer("people2")).define("people2", ["stages","d3"], function(stages,d3)
{
  ...
  main.variable(observer("nodes2")).define("nodes2", ["d3","people2","groups","radius"], function(d3,people2,groups,radius){return(
  ...
  main.variable(observer("chart2")).define("chart2", ["d3","width","height","nodes2","groups2","forceCluster2","forceCollide"], function(d3,width,height,nodes2,groups2,forceCluster2,forceCollide)
{
  ...
  svg.selectAll('.grpcnt2')
    ...
    .text(d => d3.format(",.0f")(Math.min(groups2[d].cnt*nodes[1].num/1000,groups2[d].max)));
  ...
  function timer2() {
    ...
    if (currdate < new Date(2020, 6, 1,20,30)) {
      d3.timeout(timer2, 25);
    }
  } // @end timer()
  
  
  // Start things off after a few seconds.
  d3.timeout(timer2, 3000);
